        ...
        <section id="page-content">
            <div class="container">
                ...
                <div class="page-title">
                    ...
                    <div class="breadcrumb float-left">
                        <ul>
                            ...
                            <li><a href="#"><?php echo $katrow->kat_adi; ?></a>
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div id="blog" class="grid-layout post-3-columns m-b-30" data-item="post-item">
                    ...
                    <!-- Post item-->
                    <?php
                        $sorgu  =   $db->prepare("SELECT yazi_kat_id , yazi_durum FROM yazilar INNER JOIN kategoriler ON kategoriler.id = yazilar.yazi_kat_id WHERE yazi_durum= :d AND yazi_kat_id=:kid");
                        $sorgu->execute([":d"=>1,':kid'=>$katrow->id]);
                        $toplam = $sorgu->rowCount();
                        $lim    =  9;
                        $goster = $s * $lim - $lim;


                        $sorgu = $db->prepare("SELECT * FROM yazilar INNER JOIN kategoriler ON kategoriler.id = yazilar.yazi_kat_id WHERE yazi_durum= :d AND yazi_kat_id=:kid ORDER BY yazi_tarih DESC LIMIT :goster,:lim");
                        $sorgu->bindValue(":d",(int) 1,PDO::PARAM_INT);
                        $sorgu->bindValue(":goster",(int) $goster,PDO::PARAM_INT);
                        $sorgu->bindValue(":kid",(int) $katrow->id,PDO::PARAM_INT);
